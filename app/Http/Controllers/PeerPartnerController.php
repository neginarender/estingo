...
class PeerPartnerController extends Controller
{
    ...
    public function login($id)
    {
        ...
        $shortId = \App\ShortingHub::whereRaw('JSON_CONTAINS(area_pincodes, \'["'.@$address->postal_code.'"]\')')->selectRaw('user_id as sorting_hub_id')->first('sorting_hub_id');
        $peer_codes = \App\PeerPartner::where('user_id', $user->id)->where('verification_status', 1)->where('peertype_approval', 0)->select('code', 'user_id', 'discount')->first();
        if(!empty($peer_codes)){
