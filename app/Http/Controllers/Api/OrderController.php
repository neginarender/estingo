...
class OrderController extends Controller
{
    ...
    public function new_orders($id)
    {
        ...
        $orders = Order::select('id','code','shipping_address','created_at')->whereIn('id',$order_id)->orderBy('id','desc')->get()->map(function($item){
            $orderDetail = OrderDetail::select('delivery_status')->where('order_id',$item->id)->first();
            $item->delivery_status = (!is_null($orderDetail)) ? $orderDetail->delivery_status:"pending";
