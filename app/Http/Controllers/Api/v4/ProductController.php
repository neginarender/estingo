...
class ProductController extends Controller
{
    ...
    public function getAllCategory(Request $request)
    {
        ...
        $dataArray = $categories->map(function($item) use($base_url,$productIds,$shortId){
            ...
            $item->url = $base_url."api/v4/products/category/".$item->id;
            $item->category_banner = $base_url."api/v4/products/category/".$item->banner;
            $item->category_banner = $base_url."api/v4/products/category/".$item->banner;
            ...
            $item->products = $product->map(function($data) use($base_url,$shortId,$cart_qty,$cart_id,$in_wishlist,$in_subscribed){
                ...
                if(!empty($shortId)){
                    $subscribed = checkInSubscribed($data->id,$shortId);
                    if($subscribed == 1){
                        $in_subscribed = true;
                    }
                }
                ...
                $data->links = ['details'=>$base_url."api/v4/products/".$data->id,
                          'related'=>$base_url."api/v4/products/related/".$data->id,
                          'reviews'=>$base_url."api/v4/reviews/product/".$data->id
                ];
                ...
                $data->in_subscribed = $in_subscribed;
                return $data;
            });
