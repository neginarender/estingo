...
class RecurringController extends Controller
{
    ...
    public function OrderRecurringPayment()
    {
        ...
        $sub = SubscribedOrder::where('status','1')->whereDate('end_date', '=', $end_date)->get();
        foreach($sub as $key => $value){
            if($end_date == date("Y-m-d",strtotime($value->end_date))){
                $OrderDev = Order::where('subscribed_id',$value->id)->where('order_status','delivered')->get();
                if(count($OrderDev) == $value->days){
                     $subscribed = \App\SubscribedOrder::where('id',$value->id)->update(['status'=> 2]);
        ...
        if(count($subscribed_orders) > 0){
                foreach($subscribed_orders as $key => $row){
                        ...
                        $order->sorting_hub_id = $row->sorting_id;
    ...
    public function orderRecurring(Request $request){
        ...
        try{
            ...
            $detail = SubscribedOrder::where('id',$order_subscribed->id)->first();

            $userDetail = User::where('id',$request->user_id)->first();

            if(!is_null($userDetail->email)){
            $applicant_mail = array();

            $applicant_mail['view'] = 'emails.subscribed';
            $applicant_mail['subject'] = 'Subscribed -'.$detail->subscribed_code;
            $applicant_mail['from'] = env('mail_from_address');

            $applicant_mail['content'] = " 
                Dear Customer,"."</br>".

                "Thank you for signing up with Rozana.in’s daily needs subscription. We are happy to make your life easier!"."</br>".
                "Your Subscription Id is ".$detail->subscribed_code.". This is valid from ".date('d-m-Y',strtotime($detail->start_date))." to ".date('d-m-Y',strtotime($detail->end_date)).".
                As a new member, you will enjoy the autonomy to get your daily needs"."</br>".

                "delivered to your home at a schedule convenient to you. You will also enjoy special offers and exclusive deals/ exciting new product announcements/ our unique take on the latest food trends."."</br>".

                "Connect with us: customercare@rozana.in";

                Mail::to($userDetail->email)->send(new EmailManager($applicant_mail));
            }
                ...
                $order = SubscribedOrder::where('id',$code)->first();
                    if($payment_type == "razorpay"){
                        ...
                        if($orderinfo['status'] == 'paid'){
                            ...
                            //Mail
                            $detail = SubscribedOrder::where('id',$order->id)->first();
        ...
        if(empty($refund)){
            ...
            if($refund_amount > 0){
                ...
                if($refundRequest->save()){
                    ...
                    $userDetail = User::where('id',$detail->user_id)->first();

                    if(!is_null($userDetail->email)){
                    $applicant_mail = array();

                    $applicant_mail['view'] = 'emails.unsubscribed';
                    $applicant_mail['subject'] = 'Unsubscribed -'.$detail->subscribed_code;
                    $applicant_mail['from'] = env('mail_from_address');

                    $applicant_mail['content'] = " 
                        Dear Customer,"."</br>".

                        "As per your request, we are cancelling your subscription Id ".$detail->subscribed_code." with immediate effect. Your ".$refund_amount." amount will be transferred to your Rozana wallet."."</br>".
