...
class OrderController extends Controller
{
    ...
    public function processOrder(Request $request)
    {
        ...
        $orderArray = [
            ...
            'grand_total' => $request->grand_total,    //// 'grand_total' => $request->grand_total + $shipping,
        ...
        if(!empty($request->user_id))
            ...
            //22-11-2021 Update used_referal_code value 1 in order table
    ...
    public function update_delivery_status(Request $request)
    {
        ...
        if($request->status=='delivered' && empty($request->name)){
        ...
        $no_of_delivered_item = OrderDetail::where('order_id',$request->order_id)->where('delivery_status','delivered')->whereNull('deleted_at')->select(DB::raw('COUNT(`id`) as id'))->pluck('id')->first();
        ...
        if($order->payment_status!="paid"){
            ...
            if($request->status == 'delivered' && $no_of_item==$no_of_delivered_item){
    ...
    public function orderDetail($id)
    {
		...
		if(count($vegetable)>0){
            ...
            $veg_order_detail = $veg_order_detail->map(function($item){
                ...
                $item->shipping_cost = doubleval(0);
    ...
    public function checkout_done(Request $request)
    {
        ...
        if($order->payment_status == 'paid'){
            ...
            if(!is_null(json_decode($order->shipping_address)->email)){
                $this->sendOrderMail($order);
            }
            SMSonOrderPlaced($order->id);
    ...
    //                ])->loadView('invoices.customer_invoice', compact('order','schedules'));
    //    $output = $pdf->output();
    //    file_put_contents('public/invoices/'.'Order#'.$order->code.'.pdf', $output);
    //    $array['view'] = 'emails.invoice';
    //    $array['subject'] = 'Rozana Order Placed - '.$order->code;
    //    $array['from'] = env('mail_from_address');
    //    // $array['content'] = view('emails.invoice', compact('order'))->render(); //translate('Dear Customer, A new order has been placed. You can check your order details in the invoice attached below. Please reach out to us in the case of any queries on customercare@rozana.in');
