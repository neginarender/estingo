...
class ProductController extends Controller
{
    ...
    public function show($id)
    {
        ...
        if(count($product) > 0){
            return new ProductDetailCollection($this->products_with_peer_price($product,$shortId,$self));
        }else{
            return [
                'success' => false,
    ...
    public function ProductSorting(Request $request)
    {
        ...
        if($subsubcategory_id!=null){
            $filterProducts->where('products.subsubcategory_id', $subsubcategory_id);
        }
        ...
        if($request->has('attribute') && !empty($attributes['attribute_id'])){
            ...
            return new ProductCollection($this->products_with_peer_price($filterProducts->whereIn('products.id',$test)->groupBy('products.id')->paginate(10),$shortId,$self));

        }else{
           
           
            return new ProductCollection($this->products_with_peer_price($filterProducts->groupBy('products.id')->paginate(10),$shortId,$self));
        }
    ...
    public function getAllCategory(Request $request)
    {
        ...
        $dataArray = $categories->map(function($item) use($base_url,$productIds,$shortId,$self){
            $item->category_name = trans($item->name);
            $item->url = $base_url."api/v5/products/category/".$item->id;
            $item->category_banner = $base_url."api/v5/products/category/".$item->banner;
            ...
            $item->products = $product->map(function($data) use($base_url,$shortId,$cart_qty,$cart_id,$in_wishlist,$self){
                ...
                $priceDetail = calculatePrice($data->id,$shortId,$self);
                ...
                $data->links = ['details'=>$base_url."api/v5/products/".$data->id,
                          'related'=>$base_url."api/v5/products/related/".$data->id,
                          'reviews'=>$base_url."api/v5/reviews/product/".$data->id
                ];
    ...
    public function homePageCategory(Request $request)
    {
        ...
        $self = $request->self;
        // $base_url  = "https://www.rozana.in/rozana_uat/";
        $base_url  = "https://ruraluat.rozana.in/";
        $shortId = 118;
        if(!empty($_SERVER['HTTP_SORTINGHUBID'])){
            $shortId = $_SERVER['HTTP_SORTINGHUBID'];
        }
        $mapped_categories = \App\MappedCategory::where('sorting_hub_id',$shortId)->where('status',1)->pluck('category_id');
        $categories = \App\Category::whereIn('id',$mapped_categories)->select('id','name','banner')->get();

        $dataArray = $categories->map(function($item) use($base_url,$productIds,$shortId,$self){
            $item->category_name = trans($item->name);
            $item->url = $base_url."api/v1/products/category/".$item->id;
            $item->category_banner = $base_url."api/v1/products/category/".$item->banner;
            $product = DB::table('final_products') 
            ->where(['category_id'=>$item->id,'published'=>1])
            ->where('sorting_hub_id',$shortId)
            ->limit(6)
            ->select(['product_id as id',trans('name'),'max_purchase_qty','thumbnail_image as thumbnail_img'])
            ->addSelect(DB::raw("'links'"))
            //->inRandomOrder()
            ->orderBy('created_at','desc')
            ->get();
            ...
            $item->products = $product->map(function($data) use($base_url,$shortId,$cart_qty,$cart_id,$in_wishlist,$self){
                ...
                $data->discount_percentage = json_decode($priceDetail['customer_discount']);
                $data->discount_type = json_decode($priceDetail['discount_type']);
                $data->max_purchase_qty = maxPurchaseQty($data->id,$_SERVER['HTTP_SORTINGHUBID']);
                return $data;
            });
    ...
    public function getProductAttribute(){
       ...
       if(!empty($subsubcategory_id)){
            ...
            $pricerange =   DB::table('products')
                            ->leftjoin('product_stocks','product_stocks.product_id','=','products.id')
                            ->where('products.subsubcategory_id',$subsubcategory_id)
                            ->select(DB::raw('MIN(unit_price) as min_price,MAX(unit_price) as max_price')) 
                            ->get();
            $products = Product::where('subsubcategory_id',$subsubcategory_id);
        }
        ...
        foreach ($non_paginate_products as $key => $product) {
            if($product->attributes != null && is_array(json_decode($product->attributes))){
                foreach (json_decode($product->attributes) as $key => $value) {
                    ...
                    if(!$flag){
                        $item['id'] = $value;
                        $item['name'] = trans(Attribute::where('id',$value)->first()->name);
                        $item['values'] = array();
  ...
  public function applyPartner(Request $request)
    {
            ...
            if(!empty($coupon)){
                if($coupon->peer_type == 'master'){
                    return response()->json([
                        'status' => true,
                        'message' => 'Master coupon has not been applied!',
                        'peercode'=> '' ,
                        'address'=>[(object) null]
                    ]);
                }else{
                    ...
                    $check = \App\PeerPartner::where('code',$request->peercode)->first();
    ...
    public function products_with_peer_price($products,$shortId,$self=0){
        $quantity = 0;
        ...
        $priceDetail = calculatePrice($product->id,$shortId,$self);
