...
use App\Http\Resources\v3\SearchProductCollection;
...
class ProductController extends Controller
{
    ...
    public function getAllCategory(Request $request)
    {
        ...
        $dataArray = $categories->map(function($item) use($base_url,$productIds,$shortId){
            ...
            $item->url = $base_url."api/v3/products/category/".$item->id;
            $item->category_banner = $base_url."api/v3/products/category/".$item->banner;
            ...
            $item->products = $product->map(function($data) use($base_url,$shortId,$cart_qty,$cart_id,$in_wishlist){
                ...
                $data->links = ['details'=>$base_url."api/v3/products/".$data->id,
                          'related'=>$base_url."api/v3/products/related/".$data->id,
                          'reviews'=>$base_url."api/v3/reviews/product/".$data->id
                ];
