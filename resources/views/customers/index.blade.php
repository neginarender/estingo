                                ...
                                <div class="btn-group dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-right">
                                        <li><a href="{{route('customers.login', encrypt($customer->id))}}">{{translate('Log in as this Customer')}}</a></li>
                                         ...
                                         <li><a href="#" onclick="confirm_peer_partner('{{route('customers.peer.partner', $customer->id)}}');">{{translate('Add as peer partner')}}  <i class="fa fa fa-handshake-o text-success" aria-hidden="true"></i> </a></li>
                                         @else
                                         <li><a href="#" onclick="confirm_peer_partner('{{route('customers.peer.partner', $customer->id)}}');">{{translate('Remove as peer partner')}}  <i class="fa fa fa-handshake-o text-danger" aria-hidden="true"></i> </a></li>
                                        @endif
...
@section('script')
    <script type="text/javascript">
        ...
        function confirm_peer_partner(url){
            $('#confirm_peer_partner').modal('show', {backdrop: 'static'});
            document.getElementById('confirmation_partner').setAttribute('href' , url);
        }
...
@section('modal')
    ...
    <div class="modal fade" id="confirm-unban" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
     ...
     <div class="modal fade" id="confirm_peer_partner" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>{{translate('Do you really want to add this Customer as peer partner?')}}</p>
                </div>

                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">{{translate('Cancel')}}</button>
                    <a id="confirmation_partner" class="btn btn-success btn-ok">{{translate('Proceed!')}}</a>
                </div>
            </div>
        </div>
    </div>
