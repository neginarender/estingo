...
@section('content')
    ...
    <section class="gry-bg py-2 py-lg-5">
        <div class="profile">
            <div class="container">
                <div class="row">
                    <div class="col-xxl-4 col-xl-5 col-lg-6 col-md-8 mx-auto">
                        <div class="card">
                            <div class="text-center px-35 pt-2 pt-lg-4">
                                <h1 class="heading heading-4 strong-500">
                                    {{ translate('Login to your account.')}}
                                </h1>
                            </div>
                            
                            <div class="px-3 px-lg-5 py-3 py-lg-4">
                                <div class="">
                                    <form class="form-default" role="form" action="{{ route('login') }}" method="POST">
                                        ...
                                        <div class="text-center">
                                            ...
                                            <br />
                                            <br />
                                            <a href ="{{url('/social-login/redirect/facebook')}}" class="btn btn-md w-100 loginBtn loginBtn--facebook">Login with Facebook</a>
                                            <a href ="{{url('social-login/redirect/google')}}" class="btn btn-md w-100 loginBtn loginBtn--google">Login with Gmail</a>
                                        </div>
                                    </form>
                            ...
                            <div class="text-center px-35 pb-3">
                                <p class="text-md"><a href="{{ route('user.login_otp') }}" class="strong-600">{{ translate('Login with phone')}}</a></p>
                                <p class="text-md">
                                    {{ translate('Need an account?')}} <a href="{{ route('user.registration') }}" class="strong-600">{{ translate('Register Now')}}</a>
                                </p>
                                <p class="text-md">
                                    {{ translate('Need an account?')}} <a href="{{ route('user.register_phone') }}" class="strong-600">{{ translate('Register with Phone')}}</a>
                                </p>
                            </div>
                        </div>
                    </div>
