                ...
                <div class="row cols-xs-space cols-sm-space cols-md-space">
                    <div class="col-lg-8">
                        <form action="{{ route('payment.checkout') }}" class="form-default" data-toggle="validator" role="form" method="POST" id="checkout-form">
                            @csrf
                            <input type="hidden" id="payw" name="payw" value="no" />
                            <div class="card">
...
$(document).ready(function($) {
        ...
        $(document).ready(function() {
            ...
            $('#checkbox1').change(function() {
                ...
                if(payment_method == 'razorpay'){
                    ...
                    var wallet_amount = "{{ session()->get('wallet_amount') }}";
                    if (checked) {
                        // alert('checked');
        ...
        function use_wallet(){
            ...
            $("#process_button").show();
            var total_amount = Math.fround("{{ session()->get('total_amount') }}");
            var walletamount = Math.fround("{{ session()->get('wallet_amount') }}");
            // alert(total_amount);
            // alert(walletamount);
