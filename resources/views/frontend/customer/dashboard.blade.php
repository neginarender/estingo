                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <div class="dashboard-widget text-center red-widget mt-4 c-pointer">
                                    <a href="{{ url('/wishlists') }}" class="d-block">
                            ...
                            <div class="col-md-4">
                                <div class="dashboard-widget text-center yellow-widget mt-4 c-pointer">
                                    <a href="{{ url('/purchase_history') }}" class="d-block">
                                        <i class="fa fa-building"></i>
                                        @php
                                            $orders = \App\Order::where('user_id', Auth::user()->id)->where('log',0)->get();
                                            $total = count($orders);
                                            /*foreach ($orders as $key => $order) {
                                                $total += count($order->orderDetails);
                                            }*/
                                        @endphp
                                        <span class="d-block title">{{ $total }} {{ translate('Order(s)') }}</span>
                                        <span class="d-block sub-title">{{ translate('In your order list') }}</span>
                                    </a>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <div class="form-box-content p-3">
                                        @if(Auth::user()->addresses != null)
                                            ...
                                            @if($address != null)
                                             ...
                                             @else
                                                    <span style="color: #09f">Default shipping address is not available. Please select any one address to make it default.</span>     
                                            @endif
                                        @endif
                                    </div>
                                </div>
