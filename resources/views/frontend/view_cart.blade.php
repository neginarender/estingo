                                            ...
                                            <tr class="cart-item {{ $key }}">
                                                ...
                                                <td class="product-price d-lg-table-cell">
                                                    @if(Session::has('referal_discount'))
                                                        @if(!empty($shortId))
                                                        <span class="product-price strong-600" id="p-price{{$key}}" p-price{{$key}}="{{ peer_discounted_newbase_price($cartItem['id'],$shortId) }}">{{ single_price(peer_discounted_newbase_price($cartItem['id'],$shortId)) }}</span>
                                                        <del class="old-product-price strong-400">{{ single_price($cartItem['price']) }}</del>
                                                        @else
                                                        <span class="product-price strong-600" id="p-price{{$key}}" p-price{{$key}}="{{ peer_discounted_newbase_price($cartItem['id']) }}">{{ single_price(peer_discounted_newbase_price($cartItem['id'])) }}</span>
                                                        <del class="old-product-price strong-400">{{ single_price($cartItem['price']) }}</del>
                                                        @endif
                                                   
                                                    @else    
                                                        <span class="pr-3 d-block" id="p-price{{$key}}" p-price{{$key}}="{{ $cartItem['price'] }}">
                                                    ...
                                                    @if($cartItem['digital'] != 1)
                                                        <div class="input-group input-group--style-2 pr-4" style="width: 130px;">
                                                            <span class="input-group-btn">
                                                                <button class="btn btn-number minus-button" type="button" data-type="minus" data-field="quantity[{{ $key }}]">
                                                                    <i class="la la-minus"></i>
                                                                </button>
                                                            </span>
                                                            <input type="text" name="quantity[{{ $key }}]" class="form-control h-auto input-number" placeholder="1" value="{{ $cartItem['quantity'] }}" min="1" max="{{ $cartItem['max_purchase_qty'] }}" onchange="updateQuantity({{ $key }}, this)" readonly>
                                                            <span class="input-group-btn">
                                                                <button class="btn btn-number plus-button" type="button" data-type="plus" data-field="quantity[{{ $key }}]">
                                                ...
                                                <td class="product-total">
                                                    ...
                                                    @if(Session::has('referal_discount'))
                                                    @if(!empty($shortId))
                                                    <span id="total-price{{$key}}">{{ single_price(peer_discounted_newbase_price($cartItem['id'],$shortId)*$cartItem['quantity']) }}</span>
                                                        <!-- <span>{{ single_price(peer_discounted_base_price($cartItem['id'])*$cartItem['quantity']) }}</span> -->
                                                     @else
                                                     <span id="total-price{{$key}}">{{ single_price(peer_discounted_newbase_price($cartItem['id'])*$cartItem['quantity']) }}</span>
                                                    
                                                     @endif
                                                    @else    
                                                        <span id="total-price{{$key}}">{{ single_price($cartItem['price']*$cartItem['quantity']) }}</span>
                                                    @endif
                        ...
                        <div class="row align-items-center pt-4">
                            <div class="col-md-6">
                                <a href="{{ route('home') }}" class="link link--style-3">
                                    ...
                                    {{ translate('Return to shops')}}
                                </a>
                            </div>
                            <div class="col-md-6 text-right">
                                
                                <button type="button" class="btn btn-lg remove_btn btn-danger" style="display:none;" onclick="removeMultipleProduct()">{{ translate('Remove') }}</button>
...
<script type="text/javascript">
    ...
    function updateQuantity(key, element){
        ...
        var total = (parseFloat($("#p-price"+key).attr("p-price"+key))*parseFloat(element.value));
        $("#total-price"+key).html("₹"+total.toFixed(2));
        $.post('{{ route('cart.updateQuantity') }}', { _token:'{{ csrf_token() }}', key:key, quantity: element.value,delivery_page:1}, function(data){
            updateNavCart();
