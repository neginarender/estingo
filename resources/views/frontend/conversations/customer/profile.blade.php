        ...
        <div class="modal-dialog modal-dialog-zoom" role="document">
            <div class="modal-content">
                ...
                <form class="form-default" role="form" action="{{ route('addresses.store') }}" method="POST">
                    @csrf
                    <div class="modal-body product-choose">
                        <div class="p-3">
                            <div class="row">
                                <div class="col-md-2">
                                ...
                                <div class="col-md-10">
                                    <textarea class="form-control textarea-autogrow mb-3" placeholder="{{ translate('Your Address') }}" rows="1" name="address" required  autocomplete="off"></textarea>
                                </div>
                            </div>
                                  <div class="row">
                            <div class="col-md-2">
                            ...
                            <div class="col-md-10">
                                <div class="mb-3">
                                    <select class="form-control mb-3 selectpicker" data-placeholder="{{ translate('Select your country')}}" name="country" required id="country_id" disabled>
                                        @foreach (\App\Country::where('status', 1)->get() as $key => $country)
                                            <option value="{{ $country->id }}" @php echo $country->name == 'India'?"selected":""; @endphp> {{ $country->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-10">
                                <div class="mb-3">
                                    <select class="form-control mb-3 selectpicker state_id" data-placeholder="{{ translate('Select your state')}}" name="state" required > 
                                    @foreach (\App\State::where('status', 1)->get() as $key => $state)
                                            <option value="{{ $state->id }}" @php echo Session::get('state_id') == $state->id? "selected":""; @endphp>{{ $state->name }}</option>
                                    @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        @php
                        $city = \App\City::select('name','id')->where('id',Session::get('city_id'))->first();
                        @endphp
                        <div class="row">
                            <div class="col-md-2">
                            ...
                            <div class="col-md-10">
                                <div class="mb-3">
                                    <select class="form-control mb-3 selectpicker city_id" data-placeholder="{{ translate('Your City')}}" name="city" required >
                                     </select>
                                    {{-- <input type="text" name="city" id="citytags" class="form-control typeahead mb-3 input-lg" placeholder="{{ translate('Your City')}}" value="" required=""  data-provide="typeahead"> --}}
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-10">
                                     <select class="form-control mb-3 selectpicker area_id" data-placeholder="{{ translate('Your Area')}}" name="area" required >
                                     <?php
                                     $area = \App\Area::select('area_name','pincode','area_id')->where('pincode',Session::get('pincode'))->get();
                                        if(!empty($area)){
                                            foreach($area as $k=>$v){
                                    ?>
                                            <option value="<?php echo $v['pincode'];?>"><?php echo $v['area_name']."|".$v['pincode'];?> </option>;
                                        <?php 
                                           }
