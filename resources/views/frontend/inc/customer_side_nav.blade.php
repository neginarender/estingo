    ...
    <div class="widget mb-0">
        <div class="widget-profile-box text-center p-3">
            @if (Auth::user()->avatar_original != null)
                <div class="image" style="background-image:url('{{Storage::disk('s3')->url(Auth::user()->avatar_original)}}')"></div>
            @else
                <img src="{{ static_asset('frontend/images/user.png') }}" class="image rounded-circle">
            @endif
            <div class="name">{{ Auth::user()->name }}</div>
            @if((Auth::user()->user_type == 'partner' && Auth::user()->peer_partner == '1') || (Auth::user()->user_type == 'staff' && Auth::user()->peer_partner == '1') && auth::user()->partner->verification_status == 1)
             <p>Peer Partner</p>
             <p>Peer Partner Code : {{Auth::user()->partner->code}}</p>
             <p style="color:#09f;font-weight: bold">Share Referral Code</p>
              <a href="https://api.whatsapp.com/send?&text=Greetings! Treat yourself to amazing year round discounts for all your grocery needs! Just enter my peer partner discount code {{Auth::user()->partner->code}} on the homepage only on rozana.in"><img src="{{ static_asset('frontend/images/whatsapp.png') }}" ></a> |
          ...
          <img src="http://png-2.findicons.com/files/icons/573/must_have/48/mail.png" style="width: 20px">
        </a>
            @endif
        </div>
