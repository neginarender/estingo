                    ...
                    <div class="bg-white all-category-menu    ">
                        <ul class="d-flex flex-wrap no-scrollbar">
                            @if(count($categories) > 50)
                                @for ($i = 0; $i < 11; $i++)
                                    <li>
                                        <a href="{{ route('products.category', $categories[$i]->slug) }}" class="row no-gutters align-items-center">
                                            <div class="col-md-12">
                                                <img loading="lazy"  class="cat-image" src="{{ my_asset($categories[$i]->banner) }}">
                                            </div>
                                            <div class="col-md-12 text-center">
                                                <div class="cat-name pt-1">{{ $categories[$i]->name }}</div>
                                            </div>
                                        </a>
                                    </li>
                                @endfor
                                <li class="">
                                    <a href="#more" class="row no-gutters align-items-center">
                                        ...
                                        <div class="col-md-12 text-center">
                                            <div class="cat-name pt-1">{{ translate('More Categories')}}</div>
                                        </div>
                                    </a>
                                </li>
                            @else
                                @foreach ($categories as $key => $category)
                                    <li>
                                        <a href="{{ route('products.category', $category->slug) }}" class="row no-gutters align-items-center">
                                             <div class="col-md-12 text-center">
                                                <img loading="lazy"  class="cat-image" src="{{ my_asset($category->banner) }}">
                                            </div>
                                             <div class="col-md-12 text-center">
                                                <div class="cat-name pt-1">{{  __($category->name) }}</div>
                                            </div>
                                        </a>
                                    </li>
                                @endforeach
            ...
            @foreach ($categories as $key => $category)
                ...
                <div class="mb-3 bg-white">
                    <div class="sub-category-menu active" id="more">
                        ...
                        <div class="row">
                            @foreach ($category->subcategories->whereIn('id',sortinghubProductDetails("cid")['scid']) as $key => $subcategory)
                            <div class="col-lg-4 col-6">
                                <h6 class="mb-3"><a href="{{ route('products.subcategory', $subcategory->slug) }}">{{  __($subcategory->name) }}</a></h6>
                                <ul class="mb-3">
                                    @foreach ($subcategory->subsubcategories->whereIn('id',sortinghubProductDetails("cid")['subscid']) as $key => $subsubcategory)
