...
<div class="panel">
    ...
    <input type="hidden" name="_token" value="{{ csrf_token() }}" />
         <table class="table table-striped" id="example" cellspacing="0" width="100%">
            ...
            <thead>
            <tr><td colspan="6"><button type="button" onclick="DeleteCheck()" class="btn btn-danger btn-sm">Delete</button></td>
            ...
            <tbody>
                ...
                @foreach($mapped_product as $key => $mapProduct)
                    <tr id="{{ $mapProduct->id }}">
                    <td><input type="checkbox" name="check[]" class="check" value="{{ $mapProduct->id }}" /></td>
                        <td>{{ ($key+1)}}</td>
                        <td>
                        @php 
                            $variant = "";
