            ...
            $peer_discount_check = \App\PeerSetting::where('product_id', '"'.$id.'"')->latest('id')->first();
                	...
                	@endphp
                		<tr>
                            ...
                            <td>
                				<label for="" class="control-label">{{ single_price(price($product->id,$shortId)) }}</label>
                			</td>
                			<td>
                                <label for="" class="control-label">{{ substr($peer_discount_check->customer_discount,1,-1) }}</label>
