...
@endforeach	
	...
	<div style="margin-left:auto;margin-right:auto;">
		<table class="padding">
			...
			<tr>
				<td colspan="2" class="text-center"> 
				<h4 style="margin: 0 0 3px; font-size: 16px">@lang('FRESHCARTONS RETAIL AND DISTRIBUTION PVT LTD')</h4>	
			...
			<tr>
				<td>{{translate("Order No")}}: {{ translate($order->code) }} <br></td>
				<td>{{translate("Date")}}: {{ date('d-m-Y h:i:s a', $order->date) }} <br></td>
			</tr>
			<tr>
				<td>
				...
				<td colspan="2">
					<table class="text-left sm-padding ">
						<tr style="border-bottom:dotted 1px #aaa; ">
							...
							<th class="border">{{translate("MRP")}}<br /><span class="border">{{translate("D-Price")}}</span></th>
						...
						<tr>
							...
							<td><span>₹ </span>{{ pdf_single_price($odDetail->price/$odDetail->quantity) }}<br /><span class="border"> <span>₹ </span>{{translate(pdf_single_price($odDetail->price/$odDetail->quantity - $odDetail->peer_discount/$odDetail->quantity))}}</span></td>
				...
				<td colspan="2"  >
					<table class="text-left sm-padding   ">
						...
						<tr>
							...
							<td>----</td>
							@php
							$total_gst += $total_tax_amount[$key];
							@endphp
							<td><span>₹ </span> {{ pdf_single_price($total_tax_amount[$key]) }}</td>
						</tr>
						@endforeach
			...
			<tr>
			<td class="border" style="padding: 0; width:200px; float:right;">{{translate("Total GST")}}</td>
			<td class="border" style="padding: 0"><span>₹ </span> {{ pdf_single_price($total_gst) }}</td>
			</tr>
			<tr>
			...
			@php 
				$schedules = \App\SubOrder::where('order_id',$order->id)->get();
				if(!empty($schedules)){
					$schedules = $schedules;
				}else{
					$schedules = array();
			...
			<td colspan="2" style="padding: 0">
			{{ ucfirst($schedule->delivery_name) }} Order Delivery Slot : {{ date('d-m-Y',strtotime($schedule->delivery_date)) }} {{ $schedule->delivery_time }}
				</td>
			</tr>
			@endif
			@endforeach
