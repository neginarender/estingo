	...
	<div style="margin-left:auto;margin-right:auto;">
				...
				<thead>
	                <tr class="gry-color" style="background: #eceff4;">
						...
						@if(!empty($order->referal_discount))
						    <th width="10%">{{ translate('Unit Discounted Price') }}</th>
						@endif
	                    <th width="10%">{{ translate('Tax') }}</th>
	                    <th width="20%" class="text-right">{{ translate('Total') }}</th>
	                </tr>
				</thead>
				<tbody class="strong">
	                ...
	                @foreach ($orderdetail as $key => $orderDetail)
						...
						@endphp
						@if($orderDetail->delivery_status != 'return') 
							<?php
							...
							@if ($orderDetail->product)
								<tr class="">
									...
									@if($orderDetail->add_by_admin==1)
										<span style="background:green;padding:10px;">added</span>
									@endif
									</td>
									{{-- <td>
									...
									<td class="gry-color currency"><img src="http://i.stack.imgur.com/nGbfO.png" width="6" height="8"> {{ pdf_single_price($orderDetail->price/$orderDetail->quantity) }}</td>
									@if(!empty($order->referal_discount))
									    <td class="gry-color currency"><img src="http://i.stack.imgur.com/nGbfO.png" width="6" height="8"> {{ pdf_single_price($orderDetail->price/$orderDetail->quantity - $orderDetail->peer_discount/$orderDetail->quantity) }}</td>
									@endif
									<td class="gry-color currency"><img src="http://i.stack.imgur.com/nGbfO.png" width="6" height="8"> {{ pdf_single_price($orderDetail->tax/$orderDetail->quantity) }}</td>
									<td class="text-right currency"><img src="http://i.stack.imgur.com/nGbfO.png" width="6" height="8"> {{ pdf_single_price($orderDetail->price - $orderDetail->peer_discount)}}</td>
					...
					<tr>
						...
						<td>{{ $product->name}}</td>
						<td>1</td>
						<td><img src="http://i.stack.imgur.com/nGbfO.png" width="6" height="8">{{ pdf_single_price(price($product->id,$shortId)) }}</td>
						<td><img src="http://i.stack.imgur.com/nGbfO.png" width="6" height="8">{{ pdf_single_price(peer_discounted_newbase_price($product->id,$shortId)) }}</td>
						<td><img src="http://i.stack.imgur.com/nGbfO.png" width="6" height="8">{{ pdf_single_price(0) }}</td>
						<td>0%</td>
						<td><img src="http://i.stack.imgur.com/nGbfO.png" width="6" height="8">{{ pdf_single_price(0) }}</td>
						<td><img src="http://i.stack.imgur.com/nGbfO.png" width="6" height="8">{{ pdf_single_price(peer_discounted_newbase_price($product->id,$shortId)) }}</td>
					</tr>
	    ...
	    <div style="padding:0 1.5rem;">
	        <table style="margin-left:auto;" class="text-right sm-padding small strong">
		        <tbody>
			        <tr>
			            <th class="gry-color text-left"> </span>{{ translate('Cart Value') }}</th>
