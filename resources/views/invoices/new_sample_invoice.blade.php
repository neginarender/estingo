...
@endforeach	
	...
	<div style="margin-left:auto;margin-right:auto;">
		<table class="padding">
				...
				<td colspan="2">
					<table class="text-left sm-padding ">
						<tr style="border-bottom:dotted 1px #aaa; ">
							...
							<th class="border" style="width: 200px">{{translate("Particulars")}} </br> {{translate("HSN Code")}}</th>
						...
						<tr>
							<!-- <td>1001</td> -->
							<td>{{ translate($odDetail->product->name) }} @if($odDetail->variation != null) ({{ translate($odDetail->variation) }}) @endif </br> {{ $odDetail->product->hsn_code }}</td>
							<td>{{ $odDetail->quantity }}</td>
							<td><span>₹ </span>{{ pdf_single_price($odDetail->price/$odDetail->quantity) }}</td>
							
							<td><span>₹ </span>{{translate(pdf_single_price($odDetail->price/$odDetail->quantity - $odDetail->peer_discount/$odDetail->quantity))}}</td>
			...
			@if($schedule->delivery_type!="normal" || $schedule->delivery_type!="Normal")
			<tr>
