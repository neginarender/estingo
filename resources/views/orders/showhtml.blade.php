        ...
        <div class="panel-body">
            ...
            @if(Auth::user()->staff->role->name == "Sorting Hub" || Auth::user()->staff->role->name == "Sorting Hub Manager")
                <div class="col-lg-offset-6 col-lg-3">
                    ...
                    $getDeliveryBoy = \App\DeliveryBoy::where('sorting_hub_id',$sorting_hub_id)->get();
                    $getAssignedBoy = \App\AssignOrder::where('order_id',$order->id)->first('delivery_boy_id');
                    @endphp
                    <input type="hidden" id="order_id" value="{{$order['id']}}">
                    <select class="form-control demo-select2"  data-minimum-results-for-search="Infinity" id="assign_order" @php echo ($delivery_status == 'delivered' || !is_null($getAssignedBoy))?"disabled":"" @endphp>
...
@endif
           ...
           @foreach($order->sub_orders as $skey => $schedule)
                ...
                <div class="col-lg-12 table-responsive">
                    <table class="table table-bordered invoice-summary">
                        ...
                        <tbody>
                            ...
                            $sh_id = Auth::user()['id'];
                            if(Auth::user()['user_type'] != "admin"){
                                $distributor = \App\MappingProduct::where('product_id',$orderDetail['product_id'])->where('sorting_hub_id', $sh_id)->first();   
                            }
                            ...
                            @endphp
                                ...
                                <tr>
                                    ...
                                    @if(!empty($order->referal_discount) && $orderDetail->quantity != 0)
                                    <td class="text-center">
                                    ...
                                    @if(Auth::user()['user_type'] != "admin")
                                        @if(Auth::user()->staff['role_id'] != 6)
                                    @php
                                    $check_return_request = \App\RefundRequest::where('order_detail_id',$orderDetail->id)->first();
                                    @endphp
                                    ...
                                    @if(Auth::user()['user_type'] != "admin")
                                            @if(Auth::user()->staff['role_id'] != 6)
                                                @if($orderDetail->delivery_status != 'return' && $order->order_status !='delivered' && $orderDetail->delivery_status != 'delivered')
                                                    @if(count($order->orderDetails)>1)
                                                    @if(Auth::user()->email!='cc_raebareli@rozana.in')
                                                        <td><a href="{{route('orders.removeorderproduct',$orderDetail->id)}}">{{translate('Remove')}}</a></td>
                                                        @endif
                                                    @endif
                                                @else
                                                    <td>
            ...
            <div class="clearfix">
                ...
                @php 
                    $refunded_amount = \App\RefundRequest::where(['order_id'=>$order->id,'refund_status'=>1])->sum('refund_amount');
                @endphp
