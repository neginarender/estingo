        ...
        <table class="table table-striped" id="example" cellspacing="0" width="100%">
            ...
            <thead>
                <tr>
                    ...
                    <th width="10%">{{translate('Assign')}}</th>
                </tr>
            </thead>
            <tbody>
                ...
                @endphp
                    ...
                    @php 
                        $getDeliveryBoy = \App\DeliveryBoy::where(['sorting_hub_id'=>$order->sorting_hub_id])->get();
                        $getAssignedBoy = \App\AssignOrder::where('order_id',$order->id)->first('delivery_boy_id');
                    @endphp
                    <td>
                    @if(!is_null($getAssignedBoy))
                                             @php
                                             $name = "NA";
                                             $dboy_user_id = \App\DeliveryBoy::where('id',$getAssignedBoy->delivery_boy_id)->first('user_id');
                                             if(!is_null($dboy_user_id)){
                                               
                                                $dboy_name = \App\User::find($dboy_user_id->user_id);
                                                if(!is_null($dboy_name)){
                                                    $name = $dboy_name->name;
                                                }
