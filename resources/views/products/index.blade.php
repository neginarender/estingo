                    ...
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('product', $product->slug) }}" target="_blank" class="media-block">
                                ...
                                <div class="media-body">{{ __($product->name) }}<br>{{ $product->id }}</div>
                                <div style="font-weight: bold;color: #09f">{{__($product->sku)}}</div>
                            </a>
                        </td>
                        <td>
                        ...
                        <td><label class="switch">
                                <input onchange="update_search_status(this)" value="{{ $product->id }}" type="checkbox" <?php if($product->search_status == 1) echo "checked";?> >
                        ...
                        <td><label class="switch">
                                ...
                                <span class="slider round"></span></label></td>
                        <td><label class="switch">
                                <input onchange="update_topproduct(this)" value="{{ $product->id }}" type="checkbox" <?php if($product->top_product == 1) echo "checked";?> >
                                <span class="slider round"></span></label></td>
                                        
                        <td>
                        <input type="text" name="max_purchase_limit" class = "max_purchase_limit" id = {{$product->id}} value = {{ $product->max_purchase_qty }} onkeypress="return (event.charCode !=8 && event.charCode ==0 || (event.charCode >= 48 && event.charCode <= 57))" maxlength="4" size="4"/> 
                        </td>
                        <td>
                            <div class="btn-group dropdown">
                                ...
                                <ul class="dropdown-menu dropdown-menu-right">
                                    ...
                                    @if(auth()->user()->user_type == "admin")
                                     ...
                                     <!-- By Hasan-->
                                     <!-- <li><a onclick="confirm_modal('{{route('products.delete', $product->id)}}');">{{translate('Delete')}}</a></li> -->
                                    @endif
...
@section('script')
    <script type="text/javascript">
        ...
        function update_search_status(el){
            if(el.checked){
                var status = 1;
            }
            else{
            ...
            $.post('{{ route('products.search_status') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    showAlert('success', 'Search status updated successfully');
                }
                else{
                    showAlert('danger', 'Something went wrong');
                }
        ...
        function update_topproduct(el){
            ...
            $.post('{{ route('products.topproducts') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    showAlert('success', 'Top products updated successfully');
                }
                else{
                    showAlert('danger', 'Something went wrong');
                }
        ...
        $(document).ready(function(){
            $(".max_purchase_limit").focusout(function(e){
                ...
                $.post('{{route('product.purchaselimit')}}',{_token:'{{csrf_token()}}',id:product_id,max_purchase_qty:max_purchase_qty},function(data){
                    if(data == 1){
