        ...
        <form method="POST" action="{{ route('password.email') }}">
            ...
            <div class="form-group">
                @if (\App\Addon::where('unique_identifier', 'otp_system')->first() != null && \App\Addon::where('unique_identifier', 'otp_system')->first()->activated)
                    <input id="email" type="text" class="form-control{{ $errors->has('email') ? ' is-invalid' : '' }}" name="email" value="{{ old('email') }}" placeholder="{{ translate('Phone Use With Country Code') }}" required="required">
                    <div class="form-group">
                                                <button class="btn btn-link p-0" type="button" id="input_change">{{ translate('Use Email Instead') }}</button>
                                            </div>
                @else
                    <input type="email" class="form-control{{ $errors->has('email') ? ' is-invalid' : '' }}" value="{{ old('email') }}" placeholder="{{ translate('Email') }}" name="email" required="required">
                @endif
