                ...
                @foreach($peer_partner as $key => $partner)
                    ...
                    @php $user = \App\User::find($partner->added_by); @endphp
                        <tr>
                            ...
                            <td>{{$partner->code}}</td>
                            <td>        <a href="{{route('callceter.login', encrypt($partner->id))}}" class="btn btn-primary">
                                @if($partner->verification_status == 1) {{translate('Log in as this Partner')}} @else {{translate('Log in as this User')}}  @endif</a></td>
