                ...
                <div class="form-group mb-3">
                    ...
                    <div class="col-sm-9">
                        <select class="form-control demo-select2" name="sorting_hub" id="sorting_hub">
                            ...
                            <option value="{{ $sorthub->user_id }}" @if($flash_deal->sorting_hub_id==$sorthub->user_id) selected @endif>{{ $sorthub->user->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                @php 
                $productIds = \App\MappingProduct::where(['published'=>1,'flash_deal'=>1,'sorting_hub_id'=>$flash_deal->sorting_hub_id])->pluck('product_id');
                @endphp
                <div class="form-group">
                    ...
                    <div class="col-sm-9">
                        <select name="products[]" id="products" class="form-control demo-select2" multiple required data-placeholder="{{ translate('Choose Products') }}">
                            @foreach(\App\Product::whereIn('id',$productIds)->get() as $product)
                                @php
        ...
        $(document).ready(function(){
            ...
            function get_flash_deal_discount(){
                ...
                if(product_ids.length > 0){
                    $.post('{{ route('flash_deals.product_discount_edit') }}', {_token:'{{ csrf_token() }}', product_ids:product_ids, flash_deal_id:{{ $flash_deal->id }},sorting_hub_id:$("#sorting_hub").val()}, function(data){
