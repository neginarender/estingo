...
@section('content')
<div>
    <h1 class="page-header text-overflow">{{ translate('Edit Sorting Manager') }}</h1>
</div>
<div class="row">
	<div class="col-lg-8 col-lg-offset-2">
		<form class="form form-horizontal mar-top" action="{{route('sortingmanager.update', encrypt($sortingmanager->id))}}" method="POST" enctype="multipart/form-data" id="choice_form">
			@csrf
			<input type="hidden" name="_method" value="PATCH">
			<div class="panel">
				<div class="panel-heading bord-btm">
					<h3 class="panel-title">{{translate('Sorting Manager Information')}}</h3>
				</div>
				<div class="panel-body">
					...
					<div class="form-group">
						<label class="col-lg-2 control-label">{{translate('Manager Name')}}</label>
						<div class="col-lg-7">
							<input type="text" class="form-control" name="name" value = {{$sortingmanager->user->name}} placeholder="{{ translate('Delivery Boy Name') }}" required>
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-lg-7">
							<input type="email" placeholder="{{translate('email')}}" id="user_id" name="email"  value = {{$sortingmanager->user->email}} class="form-control" disabled>
						</div>
                    </div>
					...
					<div class="form-group">
						...
						<div class="col-lg-7">
							<input type="text" placeholder="Phone" name="phone" value={{$sortingmanager->phone}} class="form-control" required>
						</div>
					</div>
